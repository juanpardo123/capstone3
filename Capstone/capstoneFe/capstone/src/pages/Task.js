...
export const Task = () => {
  ...
  useEffect(() => {
    const getUserTasks = async () => {
      const data = await fetchData(`singletask/${completedTasks.id}`);
      if (data) {
        setUserTasks(data);
          ...
          <div className='taskHolder'>
            ...
            {[currentTask].map((task) => (
              ...
              <div className='sideholder'>
                ...
                <img className='image' src={task.imageUrl} alt={task.name} />
                <p className='description'>{task.description}</p>
                </div>
                <div className='stepHolder'>
                  {task.steps.map((step) => (
                    <div key={step.id} className='step'>
                      <h3>{`Step: ${step.sequence} `+step.name}</h3>
                      <p>{step.description}</p>
                      <img className='image' src={step.imageUrl} alt={step.name} />
                    </div>
                  ))}
                </div>
              </div>
